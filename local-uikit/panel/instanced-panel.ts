...
import { Bucket } from '../../submodules/uikit/packages/uikit/src/allocation/sorted-buckets.ts'
import { ClippingRect, defaultClippingData } from '../../submodules/uikit/packages/uikit/src/clipping.ts'
import { Inset } from '../../submodules/uikit/packages/uikit/src/flex/node.ts'
import { InstancedPanelGroup, PanelGroupProperties } from './instanced-panel-group.ts'
import { abortableEffect, ColorRepresentation } from '../../submodules/uikit/packages/uikit/src/utils.ts'
import { OrderInfo } from '../../submodules/uikit/packages/uikit/src/order.ts'
import { PanelMaterialConfig } from '../../submodules/uikit/packages/uikit/src/panel/panel-material.ts'
import { BaseOutProperties, Properties } from '../../submodules/uikit/packages/uikit/src/properties/index.ts'
...
export class InstancedPanel {
  ...
  public activate(bucket: Bucket<unknown>, index: number): void {
    ...
    abortableEffect(() => {
      ...
      this.group.syncInstanceData(bufferIndex, 2)
      root.requestRender?.()
    }, this.abortController.signal)
    abortableEffect(() => {
      const index = this.getIndexInBuffer()
      ...
      instanceData.needsUpdate = true
      this.group.syncInstanceData(offset, 4)
      root.requestRender?.()
    }, this.abortController.signal),
      abortableEffect(() => {
        const index = this.getIndexInBuffer()
        ...
        this.group.syncInstanceClipping(offset, 16)
        root.requestRender?.()
      }, this.abortController.signal)
  }
